"""
Legacy FastAPI surface retained for compatibility.
Deprecated: use backend.app as the canonical backend entrypoint.
"""
import asyncio
...
DEPRECATION_WARNING = (
    "app.api.main is deprecated and will be removed in a future release. "
    "Use backend.app (python -m backend.app) instead."
)

CORE_INDEX_SYMBOLS = ("SPX", "NDX", "DJI", "RUT")


def _normalize_symbol(value: str) -> str:
    """Normalize incoming symbol/ticker to uppercase bare symbol."""
...
def _parse_symbol_list(raw_symbols: Optional[str]) -> List[str]:
    """Parse comma-separated symbols and deduplicate while preserving order."""
    if raw_symbols:
        candidates = [s.strip() for s in raw_symbols.split(",") if s.strip()]
    else:
        candidates = settings.get_databento_symbols()
...
limiter = Limiter(key_func=get_remote_address, default_limits=["120/minute"], headers_enabled=True)
...
async def add_deprecation_headers(request: Request, call_next):
    ...
    response.headers["Warning"] = '299 - "Deprecated API surface: app.api.main; use backend.app"'
    response.headers["X-Deprecated-Backend"] = "app.api.main"
    response.headers["X-Replacement-Backend"] = "backend.app"
    return response

# Per-symbol last prediction timestamp for cadence enforcement
_last_prediction_ts: Dict[str, float] = {
    s: 0.0 for s in settings.get_databento_symbols()
}
...
def _start_supervised_task(name: str, coro: asyncio.Future):
    """Create background task with exception logging."""
    ...
    def _log_exception(t: asyncio.Task):
        if t.cancelled():
            ...
            log.error(f"Task {name} crashed: {exc}", exc_info=exc)

    task.add_done_callback(_log_exception)
    return task


async def _run_blocking(func, *args, timeout: float = 1.0, **kwargs):
...
class DatabentoPredictionRow(BaseModel):
    """Snapshot-based prediction row for a single Databento ticker."""
    symbol: str
    request_ticker: str
    current_price: float
    predicted_close: float
...
class DatabentoBatchPredictionResponse(BaseModel):
    """Batch prediction response for Databento symbols."""
    symbols: List[DatabentoPredictionRow]
    count: int
    timestamp: str
...
async def startup():
    ...
    for symbol in settings.get_databento_symbols():
    ...
    _start_supervised_task("schedule_oi_refresh", schedule_oi_refresh())

    # Start aggregation flush task
    from app.ingest.websocket_aggregator import flush_aggregates
    _start_supervised_task("flush_aggregates", flush_aggregates())

    # Databento-only mode: keep Polygon WebSocket paths disabled.
    log.info("Polygon WebSocket ingestion disabled (Databento-only mode)")

    # Start Databento REST polling and DB cached snapshot fallback.
    from app.ingest.rest_fallback import poll_polygon_rest, load_cached_snapshots
    _start_supervised_task("poll_polygon_rest", poll_polygon_rest())
    _start_supervised_task("load_cached_snapshots", load_cached_snapshots())  # Secondary fallback from DB

    # Start Databento gamma streamer when key is configured.
    if settings.databento_api_key:
        try:
            from backend.databento_streamer import DatabentoGammaStreamer

            _databento_gamma_streamer = DatabentoGammaStreamer(symbols=["SPX", "NDX", "DJI", "RUT", "VIX"])
            _start_supervised_task("databento_gamma_streamer", _databento_gamma_streamer.start())
            log.info("Databento gamma streamer started")
        except ModuleNotFoundError as exc:
            log.warning("Databento gamma streamer unavailable (%s)", exc)
        except Exception as exc:
            log.warning("Databento gamma streamer failed to start: %s", exc)
    else:
        log.warning("DATABENTO_API_KEY not configured; Databento-only mode cannot stream live data")

    log.info("API ready - Databento REST polling + cached snapshot fallback")
...
async def health_check(request: Request, response: Response):
    ...
    for symbol in settings.get_databento_symbols():
        ...
        status[symbol] = {
            ...
            "mode": "REST" if rest_only else "WebSocket"
        }

    try:
        market_open = is_regular_hours(datetime.now(timezone.utc))
    except Exception:
        market_open = False

    overall_ok = all(s["fresh"] or not market_open for s in status.values())
...
async def get_gamma_levels(request: Request, symbol: str) -> LevelsResponse:
    """
    Get gamma exposure levels for symbol.
    Uses OI cache (no REST calls).
    Works after hours using fallback pricing.
    """
    if symbol not in CORE_INDEX_SYMBOLS:
...
@limiter.limit("30/minute;8/second")
async def predict_close(
    ...
    # Check if market is open unless the caller explicitly allows after-hours fallback.
    if not is_regular_hours(dt_utc) and not allow_after_hours:
    ...
    max_age = 5
    rest_only = is_rest_only_mode()
    min_data_seconds = 10 if rest_only else 300

    ring = INDEX_RINGS.get(symbol)

    # If stale, try immediate fallback refresh (Databento/REST snapshot) to unblock predictions.
    if not ring or not ring.is_fresh(max_age_seconds=max_age):
        fallback_price = get_latest_price_with_fallback(
            symbol,
            settings.databento_api_key or settings.polygon_api_key,
        )
        if fallback_price is not None and ring is not None:
            from app.state.ring_buffers import IndexTick, update_session_vwap

            ts_now = int(time.time())
            ring.add(ts_now, IndexTick(ts=ts_now, price=float(fallback_price), size=1.0))
            update_session_vwap(symbol, float(fallback_price), 1.0)

            # In REST-only mode, bootstrap minimum history from the current real index snapshot.
            if rest_only and ring.length_seconds < min_data_seconds:
                for offset in range(min_data_seconds - 1, 0, -1):
                    ts_seed = ts_now - offset
                    ring.add(ts_seed, IndexTick(ts=ts_seed, price=float(fallback_price), size=1.0))
    ...
    try:
        features = await _run_blocking(compute_all_features, symbol, timeout=0.15)
...
async def predict_eod(request: Request, symbol: str) -> EODPredictionResponse:
    """
...
@limiter.limit("20/minute")
async def get_close_predictor_overlay(request: Request, response: Response, symbol: str = "SPX") -> ClosePredictorResponse:
    """
    ...
    try:
        ...
        if is_regular_hours(now_utc):
            try:
                gex_analysis = await _run_blocking(
                    options_gamma.get_gamma_analysis,
                    timeout=1.5,
                    api_key=settings.polygon_api_key,
                    underlying=symbol,
                    spot_price=current_price
...
async def get_multi_expiry_gamma(request: Request, symbol: str = "SPX", max_dte: int = 7):
    """
    ...
    if clean_symbol not in CORE_INDEX_SYMBOLS:
        raise HTTPException(400, f"Invalid symbol: {symbol}")
...
async def get_ai_enhanced_prediction_endpoint(request: Request, response: Response, symbol: str = "SPX"):
    """
    ...
    try:
        ...
        if inputs['walls']:
            ...
            # Graceful fallback when gamma/options data is unavailable.
            class _FallbackResult:
                eod_estimate = current_price
                wwm = None
                vacp = None
                pin_stability_index = None
                zero_gamma = None

            base_result = _FallbackResult()
        ...
        base_model_response = {
            ...
            "fallback_mode": not bool(inputs['walls'])
        }
    ...
    try:
        ...
        gamma_data = await _run_blocking(
            options_gamma.get_gamma_analysis,
            timeout=1.5,
            api_key=settings.polygon_api_key,
            underlying=clean_symbol,
        ...
        multi_expiry_data = await _run_blocking(
            options_gamma.get_multi_expiry_analysis,
            timeout=2.0,
            api_key=settings.polygon_api_key,
            underlying=clean_symbol,
            spot_price=current_price,
...
async def get_market_briefing_endpoint(request: Request, symbol: str = "SPX"):
    """
...
def _snapshot_to_prediction(symbol: str, request_ticker: str, snapshot: Dict[str, Any]) -> DatabentoPredictionRow:
    """Convert Databento snapshot data into a lightweight close prediction row."""
    current_price = float(snapshot.get("price") or snapshot.get("close") or 0.0)
    prev_close_raw = snapshot.get("prev_close")
    ...
    if prev_close and prev_close > 0:
        trend_pct = ((current_price - prev_close) / prev_close) * 100.0
        # Keep adjustment conservative: project 20% of current intraday drift.
        projected_pct = max(min((trend_pct / 100.0) * 0.2, 0.02), -0.02)
        predicted_close = current_price * (1.0 + projected_pct)
        confidence = "medium"

    return DatabentoPredictionRow(
        symbol=symbol,
        request_ticker=request_ticker,
        current_price=current_price,
        predicted_close=float(predicted_close),
        confidence_level=confidence,
        source=str(snapshot.get("source", "databento")),
        proxy_symbol=snapshot.get("proxy_symbol"),
...
@app.get("/predict/databento/all")
@limiter.limit("10/minute")
async def predict_databento_all(request: Request, response: Response, symbols: Optional[str] = None) -> DatabentoBatchPredictionResponse:
    """
    Get snapshot-based predictions for SPX/NDX and all configured Databento symbols.

    - Uses `DATABENTO_SYMBOLS` from env by default.
    - Supports override via `?symbols=SPX,NDX,AAPL,MSFT`.
    - Returns one prediction row per symbol with conservative trend projection.
    """
    requested_symbols = _parse_symbol_list(symbols)
    if not requested_symbols:
        raise HTTPException(400, "No symbols requested. Set DATABENTO_SYMBOLS or pass ?symbols=")

    request_tickers = [f"I:{s}" if s in CORE_INDEX_SYMBOLS else s for s in requested_symbols]
    snapshot_data: Dict[str, Any] = {}

    try:
        # Primary path: same Databento fetch helper used by ingest polling.
        from app.ingest.rest_fallback import _fetch_databento_prices

        prices = await asyncio.to_thread(
            _fetch_databento_prices,
        ...
        if prices:
            for symbol in requested_symbols:
                if symbol in prices:
                    request_ticker = f"I:{symbol}" if symbol in CORE_INDEX_SYMBOLS else symbol
                    snapshot_data[request_ticker] = {
                        "price": float(prices[symbol]),
                        "source": "databento",
                    }

        # Fallback path: use snapshot helper for anything still missing.
        missing_tickers = [t for t in request_tickers if t not in snapshot_data]
        if missing_tickers:
            from websocket_streaming import get_snapshot_data

            fallback_snapshot = await _run_blocking(
                get_snapshot_data,
                settings.databento_api_key or settings.polygon_api_key,
                missing_tickers,
                timeout=10.0,
            ...
            if fallback_snapshot:
                snapshot_data.update(fallback_snapshot)
    except Exception as e:
        raise HTTPException(503, f"Snapshot fetch failed: {e}")

    if not snapshot_data:
        raise HTTPException(503, "No snapshot data returned for requested symbols")

    rows: List[DatabentoPredictionRow] = []
    for symbol, request_ticker in zip(requested_symbols, request_tickers):
        snapshot = snapshot_data.get(request_ticker)
        if not snapshot:
            continue
        rows.append(_snapshot_to_prediction(symbol, request_ticker, snapshot))

    if not rows:
        raise HTTPException(503, "No prediction rows available from snapshot results")

    return DatabentoBatchPredictionResponse(
        symbols=rows,
        count=len(rows),
        timestamp=now_et().isoformat(),
    )
