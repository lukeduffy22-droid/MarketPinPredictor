...
def calc_flow_urgency(symbol: str, lookback_seconds: int = 60) -> float:
    ...
    High urgency = large recent flow plus directional imbalance when side was
    explicitly inferred. Unknown-side trades contribute volume, not direction.
    ...
    if classified_notional > 0:
        directional_bias = abs(buy_notional / classified_notional - 0.5) * 2  # 0-1
    else:
