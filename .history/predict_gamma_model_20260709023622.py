...
def predict_from_dataframe(df, index_name, meta_file=None, model_file=None, output_csv=None):
    ...
    if scaling and scaling.get("mean") is not None and scaling.get("std") is not None:
    ...
    pred_distance = np.concatenate(preds_out, axis=0).flatten() if preds_out else np.array([], dtype=np.float32)
