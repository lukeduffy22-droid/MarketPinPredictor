...
from train_gamma_model import train_model
...
def main() -> int:
    ...
    if args.rebuild or not dataset_path.exists():
        print(f"Building unified dataset from {args.exports_dir}")
        df = build_dataset(args.exports_dir, include_invalid=args.include_invalid)
        if df.empty:
