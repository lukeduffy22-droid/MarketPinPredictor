...
def check_environment():
    ...
    print(f"✓ DATABENTO_API_KEY: {'SET' if databento_key else '⚠️ NOT SET (optional, preferred for market-open feed)'}")
    print(f"✓ MARKET_DATA_PROVIDER: {provider}")
    print(f"✓ DATABASE_URL: {'SET' if db_url else '⚠️ NOT SET (using SQLite fallback)'}")
    
    if not api_key and not databento_key:
        print("\n⚠️ WARNING: No live data keys set. Gamma scheduler will not collect live data.")
        print("   Set environment variable: export Massive_API='your_api_key'")
        print("   or: export POLYGON_API_KEY='your_api_key'")
        print("   and/or: export DATABENTO_API_KEY='your_api_key'")
    
    return (api_key is not None) or (databento_key is not None)
