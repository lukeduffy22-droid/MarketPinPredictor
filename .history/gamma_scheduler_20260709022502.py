...
def fetch_and_save_gamma_snapshot(api_key, symbol):
    ...
    try:
        from app.core.audit_persistence import persist_audit_snapshot, persist_validated_audit_snapshot
        ...
        # Step 3b: Persist validated-only snapshots for high-quality replay/training sets
        if is_valid_for_model:
            validated_file = persist_validated_audit_snapshot(audit_snapshot)
            if validated_file:
                print(f"  ✅ Validated snapshot recorded: {validated_file}")
